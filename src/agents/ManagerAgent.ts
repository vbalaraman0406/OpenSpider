...
export class ManagerAgent {
    ...
    async processUserRequest(prompt: string, imagesBase64: string[] = [], issuerRole: 'admin' | 'guest' = 'admin', sessionKey?: string): Promise<string> {
...
[MLB / BASEBALL / FANTASY BASEBALL ROUTING RULE]
If the user asks about MLB, baseball, fantasy baseball, Yahoo Fantasy, fantasy league, draft, lineup, roster, waiver wire, free agents, trade, player stats, pitching matchup, batting average, ERA, home runs, RBI, stolen bases, starting pitcher, closer, bullpen, spring training, minor leagues, any MLB team (Yankees, Dodgers, Red Sox, etc.), any MLB player, or ANY baseball/fantasy sports topic — you MUST ALWAYS delegate this to the "Fantasy Baseball Strategist" agent. This agent has browse_web, web_search, and web_fetch tools and specializes in Yahoo Fantasy Baseball operations. Always assign the role as "Fantasy Baseball Strategist". IMPORTANT: The user's Yahoo Fantasy account is already logged in via the Browser Relay — instruct the agent to browse directly to https://baseball.fantasysports.yahoo.com/ and click through to the user's team page. Do NOT tell the user to log in.
        ...
        try {
            ...
            for (let i = 0; i < planResult.plan.length; i++) {
                ...
                if (step.type === 'task' && step.role && step.instruction) {
                    ...
                    const worker = new WorkerAgent(workerLlm, resolvedRole, () => this.cancelRequested, isCron, analysisLlm, 'admin', sessionKey);
                    ...
                    const promises = step.subtasks.map(async (subtask, j) => {
                        ...
                        const worker = new WorkerAgent(workerLlm, resolvedRole, () => this.cancelRequested, isCron, analysisLlm, issuerRole, sessionKey);
